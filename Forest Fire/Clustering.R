...
# Remove the 'area' column from the dataset for clustering
df_cluster <- df2 %>% select(-area)

# Ensure all columns are numeric (if any factor/character columns exist, convert or exclude them)
...
for (i in 1:10) {
  set.seed(123)
  kmeans_result <- kmeans(df_cluster, centers = i)
...
kmeans_result <- kmeans(df_cluster, centers = k) # k cluster solution

# Add the cluster assignments to the original dataframe
df2$cluster <- kmeans_result$cluster


# Get the predicted 'area' as the centroid of each cluster
df2$predicted_area <- ave(df2$area, df2$cluster, FUN = mean)

# Calculate MSE (Mean Squared Error)
mse <- mean((df2$area - df2$predicted_area)^2)

# Calculate RMSE (Root Mean Squared Error)
rmse <- sqrt(mean((df2$area - df2$predicted_area)^2))

# Calculate MAE (Mean Absolute Error)
mae <- mean(abs(df2$area - df2$predicted_area))

# Calculate R-squared
sst <- sum((df2$area - mean(df2$area))^2)  # Total sum of squares
ssr <- sum((df2$predicted_area - mean(df2$area))^2)  # Explained sum of squares
r_squared <- ssr / sst
